...
const NAV_ITEMS = [
  { name: 'Home', label: 'Home', Filled: HomeFilledIcon, Outline: HomeOutlineIcon },
  { name: 'Properties', label: 'Properties', Filled: BuildingsFilledIcon, Outline: BuildingsOutlineIcon },
  { name: 'Finance', label: 'Financials', Filled: DollarFilledIcon, Outline: DollarOutlineIcon },
  { name: 'Maintenance', label: 'Maintenance', Filled: WrenchFilledIcon, Outline: WrenchOutlineIcon },
  { name: 'Profile', label: 'Profile', Filled: UserFilledIcon, Outline: UserOutlineIcon },
];

export default function CustomTabBar({ state, navigation }: BottomTabBarProps) {
  const insets = useSafeAreaInsets();
  const isWeb = Platform.OS === 'web';
  const bottom = isWeb ? 12 : insets.bottom > 0 ? insets.bottom + 4 : 12;
            ...
            <Pressable key={route.key} onPress={onPress} style={styles.tabItem}>
              {isFocused ? (
                <View style={styles.activeItem}>
                  <item.Filled color={iconColor} />
                  <Text style={styles.activeLabel}>{item.label}</Text>
                </View>
              ) : (
                <item.Outline color={iconColor} />
              )}
            </Pressable>
          );
        })}
...
const styles = StyleSheet.create({
  ...
  activeItem: { alignItems: 'center', gap: 3 },
  activeLabel: { fontWeight: '500', fontSize: 10, color: Colors.gold, lineHeight: 14 },
});
